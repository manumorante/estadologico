...
case "ampliar" ' --------------------------------------------------------------------------- Ampliar noticia
	...
	if id <> "" then
		...
		<table border="0" cellpadding="0" cellspacing="0" width="100%" align="center">
			...
			<td>
			  ...
			  <table width="100%"  border="0" cellspacing="0" cellpadding="5">
                <tr>
				  ...
				  <%end if%>
				  <span class="visor-titulo"><%=re("R_TITULO")%></span></td>
                </tr>
              </table>
			  </td>
		  ...
		  <tr>
				<td><br><span class="visor-cuerpo"><%=replace(re("R_"&nav_cuerpo),vbcrlf,"<br />")%></span></td>
		  </tr>
				<%end if
			end if%>
...
'			if re("R_ACTIVO") or not nav_activo then%>
		...
		<%if nav_ampliar then%>
			<a class="titular-atitulo" href="index.asp?ac=ampliar&id=<%=re("R_ID")%>&seccion=<%=seccion%>&pag=<%=pag%>">
		<%end if%>
		
		&nbsp;<b><img src="/<%=c_s%>admin/images/flecha.gif" border="0">
		...
		if nav_archivo then
			if re("R_ARCHIVO") <> "" then
				...
				<tr>
				<td><a href="/<%=c_s%>admin/descargas/?idi=<%=idioma%>&cualid=<%=cualid%>&id=<%=re("R_ID")%>"><img src="/<%=c_s%>admin/global/img/descargar.gif" alt=" Descargar: <%=unpoco(re("R_TITULO"),60)%> " width="79" height="18" border="0" align="absmiddle"></a></td>
				</tr>
				</table>
				<%
			end if
