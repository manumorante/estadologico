<%	nombre = request.Form("nombre")
	...
	if nombre <>"" then
		...
		%><script language="javascript" type="text/javascript">
			...
			set secciones = nodoCualid.selectSingleNode("secciones")
			...
			if not unerror then
				...
				' Campos fijos
				sql = sql & "S_NOMBRE = '"& replace(nombre,"'","''") &"'"
				if config_activo_seccion then
					sql = sql & ", S_ACTIVO = "& numero(request.Form("activo"))
				end if
				...
				if camposconfigurables then
					for each a in secciones.childNodes
						if a.nodeName = "campo" then
							sql = sql & ", S_"& ucase(a.getAttribute("nombre")) &"= '"& replace(request.Form(a.getAttribute("nombre")),"'","''") &"'"
						end if
					next
				end if
			...
			if not unerror then
				...
				select case foto
					case "anadir", "cambiar", "auto"
						redir = "archivos_frames.asp?ac=formguardarfotoseccion&id="& id &"&icono="& icono
					case "eliminar"
						redir = "archivos_frames.asp?ac=quitarfotoseccion&id="& id &"&icono="& icono
					case "nada"
						select case icono
							case "anadir","cambiar"
								redir = "archivos_frames.asp?ac=formguardariconoseccion&id="& id
							case "eliminar"
								redir = "archivos_frames.asp?ac=quitariconoseccion&id="& id
						end select
				end select
				if redir <> "" then
					Response.Redirect(redir)
				else
					Response.Redirect("main.asp?ac=adminsecciones&msg=Sección editada correctamente.")
				end if
			end if
		...
		sql = "SELECT * FROM SECCIONES WHERE S_ID = " & id
		consultaXOpen sql,1
%>
<script language="javascript" type="text/javascript">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td align="center" valign="middle" background="img/titulo_cen.gif"><b><font color="#FFFFFF">Editar: <%=config_nom_secciones%></font></b></td>
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td>Escriba el nuevo nombre para la secci&oacute;n <b><%=re("S_NOMBRE")%></b> y pulse en el bot&oacute;n aceptar.<br>
...
<%if config_activo_seccion then%>
	<td align="right" valign="top"><table  border="0" cellspacing="0" cellpadding="2">
      <tr>
        <td><input type="checkbox" name="activo" id="activo" value="1" <%if re("S_ACTIVO") then Response.Write "checked" end if%>></td>
...
<table width="100%" border="0" cellpadding="2" cellspacing="0">
  ...
  <tr>
    <td align="left"><input name="nombre" type="text" class="campoAdmin" id="nombre" style="width:100%" value="<%=re("S_NOMBRE")%>" maxlength="100"></td>
    </tr>
</table>
...
if not unerror then
	for each a in nodoSecciones.childNodes
	...
	%>
	  ...
	  <tr>
		<td colspan="2" valign="top">
		  <%select case c_tipo
		  case "text"%>
		  <input name="<%=c_nombre%>" type="text" <%if a.getAttribute("manipulable")="0" then response.Write("disabled='true'") end if%> class="campoAdmin" id="subtitulo" style="width:100%" value="<%=re("S_" & c_nombre)%>" maxlength="255">
		  <%case "memo"%>
		  <textarea name="<%=c_nombre%>" rows="<%=a.getAttribute("filas")%>" wrap="virtual" class="areaAdmin" style="width:100%"><%=re("S_" & c_nombre)%></textarea>
		<%if a.getAttribute("editorhtml") = 1 then%>
		  ...
		  <%for each opcion in a.childNodes%>
			<option value="<%=opcion.getAttribute("valor")%>" <%if re("S_" & c_nombre) = opcion.getAttribute("valor") then Response.Write "selected" end if%>><%=opcion.getAttribute("titulo")%></option>
		  <%next%>
		  </select>
...
<%if config_icono_seccion then%>
	<table width="100%"  border="0" cellspacing="0" cellpadding="2">
      ...
      <tr>
        <td bgcolor="#FFFFFF">
		<%if re("S_ICONO") <> "" then%>
		<img src="../../datos/<%=session("idioma")%>/<%=cualid%>/iconos_seccion/<%=re("S_ICONO")%>">
		<br>
